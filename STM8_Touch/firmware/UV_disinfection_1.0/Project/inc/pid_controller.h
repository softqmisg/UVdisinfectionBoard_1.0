...
void PIDInit(PIDControl *pid, float kp, float ki, float kd,
		float sampleTimeSeconds, float minOutput, float maxOutput, PIDMode mode,
		PIDDirection controllerDirection);
...
// PID setpoint  Get
// Description:
//      Returns the setpoint value the particular
// Parameters:
//      pid - The address of a PIDControl instantiation.
// Returns:
//      The input of the specific PID controller.
// 
static inline float PIDSetpointGet(PIDControl *pid) {
	return pid->setpoint;
}
